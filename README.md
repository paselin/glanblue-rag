...
- 🤖 **OpenAI互換API**: 標準的なOpenAI APIエンドポイントを提供
- 🔍 **RAG (Retrieval-Augmented Generation)**: ベクトル検索による正確な情報取得
- 🤝 **Agent**: LangChainエージェントによるインタラクティブな対話
- 💻 **ローカルLLM対応**: Ollamaを使用したプライベート環境での運用
- 📊 **マルチソース**: GameWith（評価）+ Wiki（用語・俗語）の統合
- 🚀 **高速**: requests + BeautifulSoupでMac/Windows両対応

## 📊 データソース

| ソース | 種類 | 件数 | 状態 |
|--------|------|------|------|
| **GameWith** | キャラクター評価・攻略 | ~648件 | ✅ 実装済 |
| **Wiki 用語集** | ゲーム用語定義 | ~500件 | ✅ 実装済 |
| **Wiki 俗語集** | コミュニティ用語 | ~770件 | ✅ 実装済 |

**合計**: 約1,918件のドキュメント

詳細は [データソース戦略](docs/DATA_SOURCES.md) を参照
...
### Phase 2: データ収集 ✅ 完了
- [x] GameWith完全スクレイピング実装（648件）
- [x] Wiki用語集スクレイピング（500件）
- [x] Wiki俗語集スクレイピング（770件）
- [x] requests + BeautifulSoup実装（Mac/Windows対応）
- [x] データソース戦略ドキュメント作成
